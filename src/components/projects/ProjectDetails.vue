<template>
  <div class="">
    <div class="lg:rounded-lg lg:shadow-md lg:px-8 lg:pb-6">
      ...
      <div class="mb-4">
        <p><span class="font-semibold">Created At:</span> {{ project?.createdAt?.slice(0, 10) }}</p>
        <p><span class="font-semibold">Last updated At:</span> {{ project?.updatedAt?.slice(0, 10) }}</p>
      </div>

      <div class="flex">
        <router-link :to="`/dashboard/projects/edit/${project?.id}`">
          <button
            class="bg-blue-600 px-10 py-2 rounded-md text-white mr-4 hover:bg-blue-700 focus:outline-none focus:ring focus:ring-blue-300">Edit</button>
        </router-link>
        <button @click="deleteProject(project?.id)"
          class="bg-red-600 px-10 py-2 rounded-md text-white hover:bg-red-700 focus:outline-none focus:ring focus:ring-red-300">Delete</button>
      </div>
    </div>
  </div>
</template>
