  ...
  <div class="lg:px-8 lg:pb-8 bg-white lg:shadow-md rounded-lg">
    ...
    <form @submit.prevent="submitForm">
      <div class="mb-4">
        ...
        <input type="text" v-model="formData.name" id="name" name="name"
      ...
      <div class="mb-4">
        ...
        <textarea v-model="formData.features " name="technologies" id="" cols="30" rows="5"
          class="w-full border border-gray-300 rounded-md px-4 py-2 focus:outline-none focus:ring focus:border-blue-500"></textarea>
      </div>
      <div class="mb-4">
        <h3 class="font-semibold mb-1 text-lg">Technologies</h3>
        <p class="text-sm mb-1">Note: Write tech by comma separated.</p>
        <textarea v-model="formData.techStack " name="technologies" id="" cols="30" rows="5"
          class="w-full border border-gray-300 rounded-md px-4 py-2 focus:outline-none focus:ring focus:border-blue-500"></textarea>
      </div>
      <div class="flex justify-between flex-col lg:flex-row items-start lg:items-center gap-5 mb-4">
        <div class="w-full lg:w-1/2">
          <label for="startDate" class="block text-gray-700 font-semibold mb-2 text-lg">Start Date:</label>
          <input type="date" v-model="formData.projectLength.startDate" id="startDate" name="startDate"
            class="w-full border border-gray-300 rounded-md px-4 py-2 focus:outline-none focus:ring focus:border-blue-500">
        </div>
        <div class="w-full lg:w-1/2">
          <label for="endDate" class="block text-gray-700 font-semibold mb-2 text-lg">End Date:</label>
          <input type="date" v-model="formData.projectLength.endDate" id="endDate" name="endDate"
            class="w-full border border-gray-300 rounded-md px-4 py-2 focus:outline-none focus:ring focus:border-blue-500">
        </div>
      </div>
      <div class="mb-4">
        <label for="sourceCode" class="block text-gray-700 font-semibold mb-2 text-lg">Repository link:</label>
        <input type="text" v-model="formData.sourceCode" id="sourceCode" name="sourceCode"
          class="w-full border border-gray-300 rounded-md px-4 py-2 focus:outline-none focus:ring focus:border-blue-500">
      </div>
      <div class="mb-4">
        <label for="liveLink" class="block text-gray-700 font-semibold mb-2 text-lg">Live Link:</label>
        <input type="text" v-model="formData.liveLink" id="liveLink" name="liveLink"
          class="w-full border border-gray-300 rounded-md px-4 py-2 focus:outline-none focus:ring focus:border-blue-500">
      </div>
      <div class="mb-4 flex flex-col lg:flex-row items-start lg:items-center gap-2">
        ...
        <select id="projectStatus" name="projectStatus" v-model="formData.projectStatus"
